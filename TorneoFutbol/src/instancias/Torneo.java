...
import archivosJSON.NombresArbitrosJSON;
...
public class Torneo {
	...
	private void inicializarArbitrajes() {
		...
		for (int i = 0; i < cantEquipos()/2; i++) {
			arbitros.add(new Arbitro (i,asignarNombre(i)));
		}
	...
	private String asignarNombre(int i) {
		NombresArbitrosJSON nombresArbitrosJson = NombresArbitrosJSON.leerJSON("src/archivosJSON/Nombres.JSON");
		ArrayList<String> nombresArbitros = nombresArbitrosJson.dame();
		return nombresArbitros.get(i);
	}
	...
	public void agregarArbitraje(Fecha fecha, Partido partido, int numeroArbitro) {
		...
		partido.setArbitro(numeroArbitro,arbitros.get(numeroArbitro).getNombre());
	...
	private boolean estaArbitro(int numeroArbitro) {
		for (Arbitro arb : arbitros) {
			if (arb.getNumero() == numeroArbitro)
				return true;
		}
		return false;
	}
